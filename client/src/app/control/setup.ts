...
export function initCards(collection: EntityCollection): void {
  ...
  for(const cardType of Object.keys(cardProperties)) {
    ...
    let card = new CardEntity(x, y, cardType as CardType, ElementumGame.playerSlot)
    card.rotate(angle)
    collection.add(card)
    angle += angle_inc
  }
...
export function initCardSlots(collection: EntityCollection) {
  ...
  for(let i=0; i<3; i++) {
      ...
      for(const offset_mult of [-1, 1]) {
          let yPos = ElementumGame.canvasHeight/2 + offset_mult * (card_height + ypad)/2
          let entity = new CardSlotEntity(xPos, yPos, i==0?"attack1":i==1?"attack2":"defend", offset_mult==1?ElementumGame.playerSlot:ElementumGame.enemySlot)
...
export function initElements(display: EntityCollection) {
  for(const params of [
    {name: ElementumGame.playerSlot as PlayerSlot, x: ElementumGame.canvasWidth * 3/4, y: ElementumGame.canvasHeight * 3/4, rotation: 0},
    {name: ElementumGame.enemySlot as PlayerSlot, x: ElementumGame.canvasWidth * 3/4, y: ElementumGame.canvasHeight * 1/4, rotation: Math.PI}
  ]) {
